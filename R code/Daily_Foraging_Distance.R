...
# This code uses data files that can be created using the "Identiying_Shooting_Disturbance" code, files are also provided in the data folder 
...
setwd('~/GBG_Analysis/GBG_Disturbance_Analysis')

#### 1. Read in and format GBG data ####

## 1.1. GPS data ##
# GBG - data prep and distance metric calculation #

gps_gbg_all <- read.csv("Distance_Metrics/Data/GBG_GPS_CLEAN_COMPLETE.csv")
head(gps_gbg_all)

# GBG list of shooting disturbance days, fields and IDs
gbg_shoot_id <- read.csv("GBG_INDIV_SHOOT_DATE_797m.csv")
head(gbg_shoot_id)
...
islay.field <- readOGR("Disturbance/Islay_Field/AS_ISLAY_GMS_FIELD_BOUNDARY.shp")
...
farm <- read.csv("Disturbance/Islay_Field/FARM_ID.csv")
...
## 1.4. Calculate daily distance metrics ## 
# Calculate cumulative daily distance traveled 
...
#### 2. Read in and format GWfG data ####

## 2.1 GPS data ##
gps_gwf_all <- read.csv("GWFG_GPS_CLEAN_COMPLETE_hab.csv")

# GBG list of shooting disturbance days, fields and IDs
gwf_shoot_id <- read.csv("Distance_Metrics/Data/GWFG_INDIV_SHOOT_DATE_797m.csv")
...
## 2.4 Calculate daily distance metrics ## ----
# Calculate cumulative daily distance traveled 
...
#### 3. Combine all data frames for both species ####
...
# Total distance between sequential gps points by day 
...
Logs <- read.csv("Disturbance/All_logs_cleaned.csv")
...
sex <- read.csv("GPS_TAG_SEX.csv")
islay_distance_sub2 <- dplyr::left_join(islay_distance_sub, sex, by = "Device_ID")


#### 5 Modeling impact of shooting disturbance on daily distance travelled ####

## 5.1 Prep for model run ##
...
islay_distance_sub2 <- read.csv("Foraging_Distance_Model_Data.csv")
...
## 5.2 Model 2-level disturbance classification ##
...
dist_glob <- glmmTMB(totaldist ~ Disturbance2 + Species + scale(indiv_dist) + Sex +  
                       ...
                       Species*scale(indiv_dist) + Disturbance2*Species*scale(indiv_dist) + ar1(shoot_date + 0|group_id) +
...
dist_globb <- glmmTMB(totaldist ~ Disturbance2 + Species + scale(indiv_dist) + Sex +  
                        Disturbance2*Species + Disturbance2*scale(indiv_dist) +
                        Species*scale(indiv_dist) + ar1(shoot_date + 0|group_id) +
                        (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                      data = islay_distance_sub2,
                      family = Gamma("log"), na.action = "na.fail")
...
dist_globg <- glmmTMB(totaldist ~ Disturbance2 + Species + Sex +
                        ar1(shoot_date + 0|group_id) +
                        (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                      data = islay_distance_sub2,
                      family = Gamma("log"), na.action = "na.fail")

dist_globh <- glmmTMB(totaldist ~ Species + Sex +
                        ar1(shoot_date + 0|group_id) +
                        (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                      data = islay_distance_sub2,
                      family = Gamma("log"), na.action = "na.fail")
...
df_dist_top <- as.data.frame(effect("Disturbance2:Species",dist_globd))
effects_top <- as.data.frame(allEffects(dist_globd))
teseffectstopt1
plot(allEffects(dist_globd))

df_dist_top2 <- emmeans(dist_globd, specs = ~ Disturbance2:Species, by = "Species", type = "response")
df_dist_top2
summary(dist_globd)
confint(dist_globd)

## 5.3 3-level disturbance classification ## 
shoot_glob <- glmmTMB(totaldist ~ Disturbance_shoot + Species + scale(indiv_dist) + Sex +  
                        ...
                        Species*scale(indiv_dist) + Disturbance_shoot*Species*scale(indiv_dist) + ar1(shoot_date + 0|group_id) +
                        (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                      data = islay_distance_sub2,
                      family = Gamma("log"), na.action = "na.fail")
...
shoot_globg <- glmmTMB(totaldist ~ Disturbance_shoot + Species + Sex +
                         ar1(shoot_date + 0|group_id) +
                         (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                       data = islay_distance_sub2,
                       family = Gamma("log"), na.action = "na.fail")

shoot_globh <- glmmTMB(totaldist ~ Species + Sex +
                         ar1(shoot_date + 0|group_id) +
                         (1|Winter) + (1|Device_ID) + (1|mode_hld_farm), 
                       data = islay_distance_sub2,
                       family = Gamma("log"), na.action = "na.fail")
...
effects <- as.data.frame(allEffects(shoot_globd))
top_mod_effects_3 <- predictorEffects(shoot_globd)
plot(top_mod_effects_3)

# Model checks r-squared
...
## 6.1 Overall top model - dist_globd - 2-level disturbance factor ##
...
ggsave("Distance_Metrics/Plots/USE_SUNCALC_797_DISTURBANCE_DISTANCE_GLMTMB_autocor_TOPNEW.jpeg", disturbance_plot_topmod)

## 6.2 3-level disturbance factor model plot (supplementary material) ##
...
ggsave("Distance_Metrics/Plots/USE_SUNCALC_626_DISTURBANCE_DISTANCE_GLMTMB_autocor_3level.jpeg", disturbance_plot)
